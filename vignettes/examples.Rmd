...
The package GPArotation provides many factor rotation methods. I have included as extra axes a rotation of the principal components using the Bentler method, which seems to produce axes that are more easily interpretable than the principal components themselves.

* Try to separate microglia, astrocytes_ependymal and endothelial-mural cells by dragging their labels onto the plot.
* Find principal components or rotated principal components that distinguish these cells and try dragging them onto the plot instead.
...
rotatedAxes <- GPArotation::bentlerT(X_smooth)$Th
colnames(rotatedAxes) <- paste0("R",seq_len(ncol(rotatedAxes)))

langevitour(X_smooth, zeisel$type, extraAxes=cbind(rotatedAxes))
```
...
I also demonstrate adding extra axes representing principal components and rotated principal components.
