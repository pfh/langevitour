...
We can reduce the noise in this data by repeatedly averaging each cell with its nearest neighbors.
...
for(iter in seq_len(iters)) {
    X <- lapply(seq_len(nrow(X)), \(i) colMeans(X[neighbors[i,],]))
    X <- do.call(rbind, X)
}

langevitour(X, zeiselPC$type)
```
