...
<div style="display: grid; grid-template-columns: 50% 50%; column-gap: 1em; row-gap: 1em; margin: 0 0 2em 4em;">
  ...
  <div>Set point repulsion.</div>
  <div><button onclick="setState({pointRepulsionType:'pca'})">PCA</button> <button onclick="setState({pointRepulsionType:'none'})">None</button></div>
  
  <div>Experiment with "damping" and "heat".</div>
  <div>
  ...
  <div>Hide measurements by unchecking the checkbox on their label.</div>
  <div><button onclick="setState({labelInactive:['bill_length_mm']})">Hide bill length</button> <button onclick="setState({labelInactive:[]})">Show all</button></div>
</div>
