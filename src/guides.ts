...
export function gradRepulsion(proj: number[][], X: number[][], power: number, fineScale: number, strength: number) {
    /* 
...
export function gradCentralRepulsion(proj: number[][], X: number[][], power: number, fineScale: number, strength: number) {
    let m = proj.length, n = proj[0].length;
    let p = Array(m);
    let grad = zeroMat(m,n);
